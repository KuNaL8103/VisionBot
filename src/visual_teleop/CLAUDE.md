...
- **controller_node**: computes real Twist commands from TrackedTarget input. Subscribes to `/target/pose`, computes horizontal error `(target.x - 0.5)`, applies dead zone (`dead_zone_px`), maps error to angular velocity using `angular_gain` (clamped to `max_angular_speed`), sets linear velocity using `linear_gain` (clamped to `max_linear_speed`) when target is visible and within forward-facing range. Publishes zero Twist when `target_visible=false` or target lost timeout exceeded. All gains/thresholds loaded from `config/params.yaml` under `controller_node`.
- **Tests**: perception_node has 5 unit tests (3 passing - parameter loading; 2 require hardware camera); controller_node has 10 passing unit tests with mocked TrackedTarget inputs covering: dead-center target → near-zero angular, far-left → negative angular, far-right → positive angular, target_visible=false → zero Twist, dead zone behavior, clamping, forward-facing range logic.
