...
class TestControllerNode(unittest.TestCase):
    ...
    def test_dead_center_produces_near_zero_angular(self):
        ...
        self.assertAlmostEqual(twist.angular.z, 0.0, places=5,
                               msg=f"Expected angular.z ~ 0.0, got {twist.angular.z}")
    ...
    def test_far_right_produces_negative_angular(self):
        ...
        self.assertEqual(twist.linear.x, 0.0,
                         msg=f"Expected zero linear.x for far-right target, got {twist.linear.x}")
    ...
    def test_target_not_visible_produces_zero_twist(self):
        ...
        self.assertEqual(twist.linear.x, 0.0,
                         msg=f"Expected zero linear.x when target not visible, got {twist.linear.x}")
        self.assertEqual(twist.angular.z, 0.0,
                         msg=f"Expected zero angular.z when target not visible, got {twist.angular.z}")
    ...
    def test_no_target_received_produces_zero_twist(self):
        ...
        self.assertEqual(twist.linear.x, 0.0,
                         msg=f"Expected zero linear.x with no target, got {twist.linear.x}")
        self.assertEqual(twist.angular.z, 0.0,
                         msg=f"Expected zero angular.z with no target, got {twist.angular.z}")
    ...
    def test_target_within_dead_zone_produces_zero_angular(self):
        ...
        self.assertAlmostEqual(twist.angular.z, 0.0, places=5,
                               msg=f"Expected zero angular.z within dead zone, got {twist.angular.z}")
    ...
    def test_angular_velocity_clamped_to_max(self):
        ...
        self.assertLessEqual(abs(twist.angular.z), self.node.max_angular_speed + 1e-6,
                             msg=f"Angular velocity {twist.angular.z} exceeds max {self.node.max_angular_speed}")
    ...
    def test_linear_velocity_clamped_to_max(self):
        ...
        self.assertLessEqual(twist.linear.x, self.node.max_linear_speed + 1e-6,
                             msg=f"Linear velocity {twist.linear.x} exceeds max {self.node.max_linear_speed}")
        self.assertGreaterEqual(twist.linear.x, 0.0)
